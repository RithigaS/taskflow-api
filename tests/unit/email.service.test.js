...
const mail_1 = require("../../src/config/mail");
describe("email.service", () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    it("uses mock transport in test environment", () => {
        const mailer = (0, mail_1.getMailer)();
    ...
    it("sends welcome email with correct recipient, subject and body", async () => {
        ...
        await email_service_1.default.sendWelcomeEmail("rithi@example.com", "Rithi", "verify123");
    ...
    it("sends reset password email with correct recipient, subject and body", async () => {
        const mailer = (0, mail_1.getMailer)();
        const spy = jest.spyOn(mailer, "sendMail");
        await email_service_1.default.sendResetEmail("rithi@example.com", "Rithi", "reset123");
        expect(spy).toHaveBeenCalledTimes(1);
        const payload = spy.mock.calls[0][0];
        expect(payload.to).toBe("rithi@example.com");
    ...
    it("sends task assignment email with correct recipient, subject and body", async () => {
        const mailer = (0, mail_1.getMailer)();
        const spy = jest.spyOn(mailer, "sendMail");
        await email_service_1.default.sendTaskAssigned("rithi@example.com", "Finish API");
        expect(spy).toHaveBeenCalledTimes(1);
        const payload = spy.mock.calls[0][0];
        expect(payload.to).toBe("rithi@example.com");
    ...
    it("sends daily digest email with overdue task list", async () => {
        const mailer = (0, mail_1.getMailer)();
        const spy = jest.spyOn(mailer, "sendMail");
        await email_service_1.default.sendDailyDigest("rithi@example.com", [
            { title: "Overdue 1" },
            { title: "Overdue 2" },
        ]);
        expect(spy).toHaveBeenCalledTimes(1);
    ...
    it("never sends real email in test environment", async () => {
        const mailer = (0, mail_1.getMailer)();
        const spy = jest.spyOn(mailer, "sendMail");
        await email_service_1.default.sendWelcomeEmail("test@example.com", "Test", "token999");
